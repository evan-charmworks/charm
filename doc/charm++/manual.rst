...
load balancing is to occur, using the *+LBPeriod {period}* runtime
parameter or the *LBManager::SetLBPeriod(double period)* call to
...
#. **Periodic load balancing mode**: This mode uses a timer to perform
   ...
   *period* argument specifying the minimum time between consecutive
   ...
   load balancing migration are only called after AtSync() and before
   ResumeFromSync(), so they can make some assumptions about state).
...
-  **LBManager::SetLBPeriod(double s)**: The **SetLBPeriod** function
   ...
   *periodic load balancing mode*. Here is how to use it:
   ...
   .. code-block:: c++
      ...
      // Or, since it's a static member function:
