...
public:
  ...
  struct FutureModel {
    ...
    FutureModel() : cur_stats(0), start_stats(0) { predictor = new DefaultFunction(); }

    FutureModel(int n) : cur_stats(0), start_stats(0), collection(n)
    {
      predictor = new DefaultFunction();
    }

    FutureModel(int n, LBPredictorFunction* myfunc)
        : cur_stats(0), start_stats(0), collection(n)
    {
      predictor = myfunc;
    }
