  ...
  void pup(PUP::er& p)
  {
    ...
    {  // binSize needs update
      for (int i = 0; i < amaps.size(); i++)
        if (amaps[i]) amaps[i]->compute_binsize();
    }
  }
};
...
public:
  ...
  void populateInitial(int arrayHdl, CkArrayOptions& options, void* ctorMsg, CkArray* mgr)
  {
    ...
    for (int index = 0; index < maxIndex; index++)
    {
      ...
      if (PE == thisPe) mgr->insertInitial(idx, CkCopyMsg(&ctorMsg));
    }
...
void _propMapInit(void)
{
  speeds = new int[CkNumPes()];
  int i;
  for (i = 0; i < CkNumPes(); i++) speeds[i] = 1;
}
#endif
/**
...
CkMigratable::~CkMigratable()
{
  ...
  if (barrierRegistered)
  {
    ...
    if (usesAtSync) myRec->getSyncBarrier()->removeClient(ldBarrierHandle);
  }
...
{
  if (usesAtSync) myRec->getSyncBarrier()->atBarrier(ldBarrierHandle);
}
...
void CkMigratable::AtSync(int waitForMigration)
{
  ...
  if (_lb_psizer_on || _lb_args.metaLbOn())
  {
    ...
    this->virtual_pup(ps);
    if (_lb_psizer_on) setPupSize(ps.size());
    if (_lb_args.metaLbOn()) myRec->getMetaBalancer()->SetCharePupSize(ps.size());
  }
...
#  endif
  ...
  lbmgr = mgr->getLBMgr();
  if (_lb_args.metaLbOn()) the_metalb = mgr->getMetaBalancer();
#  if CMK_GLOBAL_LOCATION_UPDATE
...
bool CkLocRec::invokeEntry(CkMigratable* obj, void* msg, int epIdx, bool doFree)
{
  ...
  { /* Tracing: */
    ...
    if (_entryTable[epIdx]->traceEnabled)
    {
      ...
      if (_entryTable[epIdx]->appWork) _TRACE_BEGIN_APPWORK();
    }
  }
#endif
  ...
  { /* Tracing: */
    if (_entryTable[epIdx]->traceEnabled)
    {
      if (_entryTable[epIdx]->appWork) _TRACE_END_APPWORK();
      _TRACE_END_EXECUTE();
    }
  }
#endif
      ...
      bounds(opts.getBounds())
{
  ...
  if (map == nullptr) CkAbort("ERROR! Local branch of array map is NULL!");
  ...
  if (cache == nullptr) CkAbort("ERROR! Local branch of location cache is NULL!\n");
...
void CkLocMgr::inform(const CkArrayIndex& idx, CmiUInt8 id, int nowOnPe)
{
  ...
  if (CkInRestarting())
  {
    ...
    if (origPe == CkMyPe())
    {
      if (maskedID >= idCounter) idCounter = maskedID + 1;
    }
    else
    {
      if (origPe < CkNumPes()) thisProxy[origPe].updateLocation(idx, id, nowOnPe);
    }
  }
  ...
  if (itr != bufferedLocationRequests.end())
  {
    for (int pe : itr->second)
    {
      ...
      if (pe != CkMyPe()) thisProxy[pe].updateLocation(idx, id, nowOnPe);
    }
...
#endif
        if (!duringDestruction) thisProxy[home].reclaimRemote(rec->getIndex(), CkMyPe());
  }
  delete rec;
}
...
#  endif
  ...
  if (msg->array_hops() > 1) multiHop(msg);
...
void CkLocMgr::emigrate(CkLocRec* rec, int toPe)
{
  ...
  for (auto itr = managers.begin(); itr != managers.end(); ++itr)
  {
    CkMigratable* el = itr->second->getEltFromArrMgr(rec->getID());
    if (el)
      if (!el->isInCore) CooBringIn(el->prefetchObjID);
  }
#endif
...
bool CkLocMgr::isRemote(const CkArrayIndex& idx, int* onPe) const
{
  ...
  if (pe == -1 || pe == CkMyPe()) return false;
...
{  // Find and register with the load balancer
  ...
  if (lbmgr == nullptr) CkAbort("LBManager not yet created?\n");
  DEBL((AA "Connected to load balancer %p\n" AB, lbmgr));
  if (_lb_args.metaLbOn())
  {
    the_metalb = (MetaBalancer*)CkLocalBranch(metalbID_);
    if (the_metalb == 0) CkAbort("MetaBalancer not yet created?\n");
  }
  syncBarrier = CkSyncBarrier::object();
  if (syncBarrier == nullptr) CkAbort("CkSyncBarrier not yet created?\n");
