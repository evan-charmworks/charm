...
void CkLocMgr::deleteManager(CkArrayID id, CkArray* mgr)
{
  ...
  if (managers.size() == 0) delete this;
}
...
void CkLocMgr::requestLocation(CmiUInt8 id, const int peToTell, bool suppressIfHere)
{
  ...
  if (suppressIfHere && peToTell == CkMyPe()) return;
...
void CkLocMgr::inform(const CkArrayIndex& idx, CmiUInt8 id, int nowOnPe)
{
  ...
  if (itr != bufferedLocationRequests.end())
  {
         ...
         i != itr->second.end(); ++i)
    {
      ...
      if (peToTell != CkMyPe()) thisProxy[peToTell].updateLocation(idx, id, nowOnPe);
    }
